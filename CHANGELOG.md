...
### Toast 自动换行 + 显示时长按内容长度自适应
- 🟡 **toast 自动换行**:CSS `white-space: nowrap` → `normal` + `word-break: break-word` + `line-height: 1.55`,长内容(如"19 个检查失败+首个失败原因")在 86vw 宽度内自动换行展示
- 🟡 **显示时长按内容长度自适应**:固定 2.6 秒 → `Math.min(15000, Math.max(3000, len * 60))`(最少 3 秒,每字 60ms,最长 15 秒),长 toast 看得完,短 toast 也不拖沓
