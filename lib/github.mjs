...
export async function fetchRepoInfo({ owner, repo, token, proxy }) {
  ...
  const r = await requestJson(apiPath, { token, proxy });
  if (!r.ok) return r;
  return { ok: true, info: { fullName: r.data.full_name, desc: r.data.description, private: r.data.private, stars: r.data.stargazers_count ?? null, pushedAt: r.data.pushed_at || null } };
}
